...
export async function createAccomplishment({ userId, text, sigilKey, occurredOn }) {
  ...
  if (sigilKey) {
    await _tagAccomplishment(supabase, userId, win.id, sigilKey);
  }

  return win;
}

export async function updateAccomplishment({ userId, id, text, sigilKey, occurredOn }) {
  const supabase = await createClient();

  const { error } = await supabase
    .from('accomplishments')
    .update({ text, occurred_on: occurredOn })
    .eq('id', id)
    .eq('user_id', userId);
  ...
  await supabase
    ...
    .delete()
    .eq('accomplishment_id', id)
    .eq('user_id', userId);

  if (sigilKey) {
    await _tagAccomplishment(supabase, userId, id, sigilKey);
  }
}

export async function deleteAccomplishment({ userId, id }) {
  const supabase = await createClient();
  const { error } = await supabase
    .from('accomplishments')
...
async function _tagAccomplishment(supabase, userId, accomplishmentId, sigilKey) {
